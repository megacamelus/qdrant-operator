...
func Labels(ref ctrl.Object) map[string]string {
	return map[string]string{
		qdrant.KubernetesLabelAppName:      qdrant.QdrantAppName,
		qdrant.KubernetesLabelAppInstance:  ref.GetName(),
		qdrant.KubernetesLabelAppComponent: qdrant.QdrantComponentInstance,
...
func AppSelector() (labels.Selector, error) {
	appName, err := labels.NewRequirement(qdrant.KubernetesLabelAppName, selection.Equals, []string{qdrant.QdrantAppName})
	if err != nil {
		return nil, err
	}
